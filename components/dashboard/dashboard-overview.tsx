...
export interface DashboardOverviewProps {
  ...
  kpis: {
    ...
    highMatchBandi: number
    toCollect: number
    toPay: number
  }
  contractTrendData: { month: string; contratti: number; valore: number }[]
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {loading ? (
          Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="glass-card rounded-2xl p-5 border border-border/20">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <ChartContainer config={{ contratti: { label: "Contratti", color: "hsl(175, 60%, 50%)" }, valore: { label: "Valore", color: "hsl(185, 60%, 40%)" } }} className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={contractTrendData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="contratti" stroke="hsl(175, 60%, 50%)" strokeWidth={2} fillOpacity={1} fill="url(#colorContratti)" />
              </AreaChart>
            </ResponsiveContainer>
          </ChartContainer>
        </motion.div>
        ...
        >
          ...
          <ChartContainer config={{ value: { label: "Contratti" } }} className="h-[200px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={riskDistribution} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={4} dataKey="value" nameKey="name">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <ChartContainer config={{ count: { label: "Scadenze", color: "hsl(175, 60%, 50%)" } }} className="h-[180px] w-full">
            <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <div className="divide-y divide-border/10">
            {contracts.slice(0, 4).map((contract) => (
              <Link key={contract.id} href={`/dashboard/contracts/${contract.id}`} className="flex items-center gap-4 px-5 py-3 hover:bg-muted/20 transition-colors">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className={`text-xs px-2.5 py-1 rounded-full border ${
                    ...
                    {contract.status === "active" ? "Attivo" : contract.status === "expiring" ? "In Scadenza" : (contract.status ?? '').replace(/^\w/, (c: string) => c.toUpperCase())}
                  </span>
      ...
      >
        <div className="px-5 py-4 border-b border-border/20 flex items-center justify-between">
          ...
          <Link href="/dashboard/alerts" className="text-sm text-primary hover:text-primary/80 transition-colors flex items-center gap-1">
            Gestisci<ArrowRight className="size-3.5" />
          </Link>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4">
          {alerts.slice(0, 4).map((alert) => (
            <div key={alert.id} className={`flex items-start gap-3 p-4 rounded-xl transition-colors ${alert.status === "handled" ? "bg-muted/10" : "bg-muted/25"}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-medium truncate ${alert.status === "handled" ? "text-muted-foreground" : "text-foreground"}`}>{alert.title}</div>
                <div className="text-xs text-muted-foreground mt-0.5 truncate">{alert.description}</div>
                {alert.trigger_date && <div className="text-xs text-muted-foreground/70 mt-1">Scadenza: {formatDate(alert.trigger_date)}</div>}
              </div>
            </div>
          ))}
        </div>
